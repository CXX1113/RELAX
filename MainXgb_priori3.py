...
initial_memory_threshold = 182050
...
for k in range(episodes):
    ...
    if k % 100 == 0:
        ...
        if (k % 300 == 0 and k > 5600):
            # print("K",k)
            r_val = validate(agent_val, observation_dim, action_dim, balance_factor, max_steps, model, X_val, Y_val,
                             val_size)
            acc_val.append(r_val)
            np.savetxt("acc_val0.08_step3.txt", acc_val)
            r_test = validate(agent_val, observation_dim, action_dim, balance_factor, max_steps, model, X_test, Y_test,
                              val_size)
            ...
            print('{0:5s} R:{1:.4f} r100:{2:.4f} rtr{3:.2f} rval{4:.2f} rtest{5:.2f}'.format(str(k),
                                                                                             total_reward / (k + 1),
                                                                                             np.array(
                                                                                                 returns[-100:]).mean(),
                                                                                             np.array(
            ...
            print('{0:5s} R:{1:.4f} r100:{2:.4f} rtr{3:.2f}'.format(str(k), total_reward / (k + 1),
                                                                    np.array(returns[-100:]).mean(),
                                                                    np.array(label[-100:]).mean()))
        else:
            pass
